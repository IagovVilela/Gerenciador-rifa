...
import { saveReservations, loadReservations, saveRifaConfig, loadRifaConfig, subscribeToReservations } from './services/dataService';
...
const RifaManager = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const config = await loadRifaConfig();
        if (Object.keys(config).length > 0) {
          setRifaConfig(config);
        }
        ...
        const unsubscribe = subscribeToReservations((data) => {
          ...
          if (data.config) {
            setRifaConfig(data.config.data || rifaConfig);
          }
        });
  ...
  // Salvar dados no Firebase sempre que houver mudanças
  useEffect(() => {
    if (!isLoading && isOnline) {
      const saveData = async () => {
        try {
          await saveReservations(reservations);
          await saveRifaConfig(rifaConfig);
        } catch (error) {
  ...
  }, [reservations, rifaConfig, isLoading, isOnline]);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center p-4">
        <div className="bg-white rounded-lg shadow-2xl p-8 w-full max-w-md text-center">
          ...
          <p className="text-gray-600 mt-2">Conectando ao Firebase</p>
        </div>
      </div>
    );
  }
        ...
        <div className="bg-white rounded-lg shadow-2xl p-8 w-full max-w-md">
          <div className="text-center mb-6">
            ...
            <div className={`flex items-center justify-center gap-2 mt-4 text-sm ${
              ...
              {isOnline ? 'Conectado ao Firebase' : 'Modo offline'}
            </div>
          </div>
