...
import { Search, Plus, Edit3, Trash2, Download, RotateCcw, Users, Hash, DollarSign, Eye, EyeOff, Lock, Unlock, Wifi, WifiOff } from 'lucide-react';
...
const RifaManager = () => {
  ...
  // URL do servidor (você pode mudar para seu servidor)
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Carregar reservas
        const reservationsResponse = await fetch(`${SERVER_URL}/api/reservations`);
  ...
  const saveToServer = async (data, endpoint) => {
    if (!isOnline) return;
  ...
  useEffect(() => {
    if (Object.keys(reservations).length > 0) {
      saveToServer({ reservations }, '/api/reservations');
    }
  }, [reservations, isOnline]);

  // Salvar configuração sempre que houver mudanças
  useEffect(() => {
    if (Object.keys(rifaConfig).length > 0) {
      saveToServer({ config: rifaConfig }, '/api/config');
    }
  }, [rifaConfig, isOnline]);
  ...
  const resetRifa = async () => {
    ...
    if (window.confirm('Confirma o reset completo da cartela? Esta ação não pode ser desfeita!')) {
      if (isOnline) {
        try {
          await fetch(`${SERVER_URL}/api/reset`, { method: 'DELETE' });
        } catch (error) {
          console.error('Erro ao resetar no servidor:', error);
        }
      }
